...
string login()
{
	...
	gotoxy(57,6);
	cout<<"UserName either admin or user";
	do{
	gotoxy(20,10);
	SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xF );
...
void Person::multidelete( )
{
	...
	for(int i=1;i<=map1.size();i++)
		...
		gotoxy(0,0);
		SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xF );
		cout<<"enter button for select  backspace button for deselect  press @  for delete \necs to exit ";
...
void Person::get_name(Person *person)                           // for name 
{
	...
	{ 
	  ...
	  line1=ch;line2=ch2;
	  map1.insert(pair<string,Person*> (line1,person));
	  map2.insert(pair<string,Person*> (line2,person));
	}	
...
multimap<string,Person*>::iterator Person::searchbyname()       // for search by name
{
	...
	multimap<string,Person*>::iterator  itr=map1.find(line1);
	multimap<string,Person*>::iterator  itr2=map2.find(line2);
	
	if(f)                                                       // if only one word is entered
	{
    	itr2=map2.find(line1);
    	if(itr==map1.end() && itr2==map2.end() )                //not present in both maps return map.end (null)
	        return map1.end();
	    ...
	    if(itr!=map1.end() && itr2==map2.end())                 //if present in map1 and not in map2
	    {
	      ...
	      else                                                  //more than one draw and ask
	      {
	          ...
	          SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xF );
		      gotoxy(4,count*2+11);
		      cout<<"press the number which one do you want   :        ";
		      int n;
	          SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xA );
		      gotoxy(47,count*2+11);
		      cin>>n;
		      while(n>count)  
		      { 
		    	gotoxy(70,count*2+8);
			    SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xC );
			    cout<<"enter between range  1 to  "<<count;
		        gotoxy(67,count*2+9);
		        cout<<" if you want to exit enter press 0         ";
		        gotoxy(47,count*2+11);
			    cout<<"               ";
			    gotoxy(47,count*2+11);
		        SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xA );
		      ...
		      multimap<string,Person*>::iterator  itrn=itr;
		       for(i=1;i<n;itrn++) i++;	
        ...
        if(itr!=map1.end() && itr2!=map2.end())   //if present i n both maps   ex:a b,b a; search for 'a' should search in both maps
        {
		      ...
		      else                                              //if more draw and ask
		      {
			      ...
			      { 
			       ...
			       multimap<string,Person*>::iterator  itrn=map4.begin();
			       for(i=1;i<n;itrn++) 
				     i++;
    ...
    { 
	   ...
	   {
	   	 if(itrt->second->lname==line2)
	         { 
			 count++; 
			 map4.insert(pair<string,Person*>( itrt->first,itrt->second));
			 }
       } 
...
void Person::deletephoneno()                                    // deleting contact
{
	system("cls");
	int ch;
	multimap<string,Person*>::iterator  itr=search1();
	Person *itr2=itr->second;
...
void Person::changedetails()                                    //modifing data
{
	...
	{
		do{
	    p->phoneno=itr->second->phoneno;
	    p->fname=itr->second->fname;
	    p->lname=itr->second->lname;
	    p->email=itr->second->email;
			system("cls");
			
          SetConsoleTextAttribute( GetStdHandle( STD_OUTPUT_HANDLE ), 0xF );
		  cout<<"\n[1].change in name  [2].in phonenumber   [3].change in email  [4].exit   : ";
		  c=getch();
		  cout<<c;
		  flage=0;
		  switch(c)
		  {
			...
			case '3':{
				...
				map3.insert(pair<string,Person*> (p->phoneno,p));
				map1.insert(pair<string,Person*> (p->fname,p));
		  		if(line!="-")map2.insert(pair<string,Person*> (p->lname,p));
				break;
			  }
			case '4': return ;
